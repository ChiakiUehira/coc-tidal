...
var BasicsList = class extends import_coc3.BasicList {
  constructor(nvim) {
    super(nvim);
    this.name = "basics";
    ...
    this.addAction("append", async (item) => {
      ...
      if (!doc || !doc.attached) {
        import_coc3.window.showMessage(`Current document not attached.`);
        return;
      }
      const edits = [];
...
var SamplesList = class extends import_coc4.BasicList {
  constructor(nvim) {
    super(nvim);
    this.name = "samples";
...
async function activate(context) {
  const enable = import_coc5.workspace.getConfiguration("tidal").get("enable");
  if (enable)
    return;
  const samples = getSamples();
  const basics = getBasics();
  context.subscriptions.push(import_coc5.listManager.registerList(new samples_default(import_coc5.workspace.nvim)));
  context.subscriptions.push(import_coc5.listManager.registerList(new basics_default(import_coc5.workspace.nvim)));
  context.subscriptions.push(import_coc5.sources.createSource({
