...
function getSamples() {
  const path = import_coc.workspace.getConfiguration("tidal").get("samplePath");
  try {
    const readSamples = import_fs.default.readdirSync(path);
    const samples = readSamples.filter((sample) => !ignores.includes(sample));
    return samples;
  } catch (err) {
    import_coc.window.showErrorMessage("coc-tidal: Samples couldn\u2019t load");
    return [];
  }
}
...
var DemoList = class extends import_coc2.BasicList {
  constructor(nvim) {
    super(nvim);
    this.name = "samples";
    ...
    this.addAction("append", async (item) => {
      ...
      if (!doc || !doc.attached) {
        import_coc2.window.showMessage(`Current document not attached.`);
        return;
      }
      const edits = [];
...
var samples_default = DemoList;

// src/index.ts
async function activate(context) {
  const enable = import_coc3.workspace.getConfiguration("tidal").get("enable");
  if (enable)
    return;
  const samples = getSamples();
  context.subscriptions.push(import_coc3.listManager.registerList(new samples_default(import_coc3.workspace.nvim)));
  context.subscriptions.push(import_coc3.sources.createSource({
