...
async function activate(context) {
  const enable = import_coc4.workspace.getConfiguration("tidal").get("enable");
  if (enable)
    return;
  const samples = getSamples();
  const basics = getBasics();
  context.subscriptions.push(import_coc4.listManager.registerList(new samples_default(import_coc4.workspace.nvim)));
  context.subscriptions.push(import_coc4.sources.createSource({
